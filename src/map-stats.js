...
function draw1(data) {
  ...
  function resize() {
    ...
    svg
      ...
      .attr(
        "transform",
        "translate(" + margin.left + "," + margin.axisMargin + ")"
      )
  ...
  svg
    ...
    .on("mousemove", function () {
      ...
      d3.selectAll("#chart2 .bar-line").transition().style("stroke", "#8F223A")
  ...
  xAxis
    .call(d3.axisTop(x_scale).tickSizeInner(10).tickSizeOuter(0).tickPadding(5))
    .call((g) => g.select(".domain").remove())
  // .call(g => g.selectAll('text').style('font-size', '15'))

  var y_scale = d3.scaleBand().domain(countries).range([0, height]).padding(0)

  var yAxis = svg.append("g").attr("class", "x-axis")
  yAxis
    .attr("transform", `translate(${-5},${0})`)
    .call(d3.axisLeft(y_scale).tickPadding(5))
    .call((g) => g.select(".domain").remove())
  ...
  new ScrollMagic.Scene({
    ...
    // .addIndicators()
    .on("enter", function () {
      let lineinterest = d3.selectAll(
        d3.selectAll(".bar-line").nodes().slice(0, 5)
      )
      lineinterest
        .transition()
        .duration(500)
    ...
    .on("leave", function () {
      let lineinterest = d3.selectAll(
        d3.selectAll(".bar-line").nodes().slice(1, 5)
      )
      lineinterest
        .transition()
        .duration(500)
  ...
  new ScrollMagic.Scene({
    ...
    .on("leave", function () {
      focus.select("text").transition().duration(300).attr("opacity", "0")
  ...
  new ScrollMagic.Scene({
    ...
    .setTween(
      new TimelineMax().to(
        ".note--1",
        1,
        { left: "50%", top: "-50%", ease: Power0.easeNone },
        0
      )
    )
    .addTo(controller)
